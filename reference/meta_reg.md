...
Moderator columns are read from the same study-level dataset used for
the primary meta-analysis. A study-label column is required and must
match the fitted studies. Continuous moderators must be numeric;
categorical moderators may be character or factor. Subgroup analysis is
not required to fit meta-regression.
